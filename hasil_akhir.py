...
def gambar():
    ...
    if 'loggedin' in session and request.method == 'POST':
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT prediction FROM hasil WHERE result = %s', (filename,))
        hasil_prediksi = cursor.fetchone()
        return render_template('image.html', filename = filename, hasil_prediksi = hasil_prediksi)    
    return render_template('image.html')#, img = imagelist)
        # We need all the account info for the user so we can display it on the profile page
        #cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        #cursor.execute('SELECT * FROM accounts WHERE id = %s', (session['id'],))
        #account = cursor.fetchone()
        ...
        #img_path = '/home/pi/project/static/luka/' + imgfile.filename
        #imgfile.save(img_path)
        #imagelist = os.listdir('static/luka/')
...
def result(page):
    ...
    #data = list(cursor.fetchall())
        ...
        cursor.execute('SELECT * FROM hasil limit %s, %s', (startat, perpage))
        tes = cursor.fetchall()
        return render_template("prediksi.html", value = tes, next_page = next_page, prev_page = prev_page)
